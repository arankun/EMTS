...
Our controller selector implementation was inspired by 
    Mike Wasson's MSDN blog post "ASP.NET Web API: Using Namespaces to Version Web APIs" 
(http://blogs.msdn.com/b/webdev/archive/2013/03/08/using-namespaces-to-version-web-apis.aspx). 
    The implementation borrowed heavily from the official Microsoft CodePlex NamespaceControllerSelector 
    sample referenced in that blog post. We made some simplifications 
    (e.g., eliminated the code that checked for duplicate paths), and we made some enhancements 
    required to address changes in the ASP.NET Web API framework (e.g., note the use of IHttpRouteData subroutes), 
but the basic design and implementation comes straight from CodePlex.
*/
